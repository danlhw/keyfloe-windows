...
import { shouldUseKeyfloeAPI } from "./keyfloe.api";

// Keyfloe STT function
async function fetchKeyfloeSTT(audio: File | Blob): Promise<string> {
  try {
    ...
    return `Keyfloe STT Error: ${errorMessage}`;
  }
}
...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    // Check if we should use Keyfloe API instead
    const useKeyfloeAPI = await shouldUseKeyfloeAPI();
    if (useKeyfloeAPI) {
      return await fetchKeyfloeSTT(audio);
    }
